...
public final class EventTapManager {
    ...
    private let monitor: ButtonMonitor
    ...
    public init(
        ...
        // Entering or leaving test mode abandons whatever gesture was in flight, so a
        // button held across the transition can't resume against half-consumed state.
        monitor.onActiveChange = { [weak self] _ in
            self?.engine.reset()
        }
    }
    ...
    private func attachTap() {
        ...
        if tap == nil {
            NSLog("[MouseEnhancer] Keyboard tap refused — falling back to mouse-only")
            tap = createTap(mouseMask)
        }
        guard let tap else {
            NSLog("[MouseEnhancer] Failed to create event tap — Accessibility permission missing?")
            waitForPermission()
            return
        }
    ...
    private func handleLostPermission() {
        NSLog("[MouseEnhancer] Accessibility access lost — waiting for it to be restored")
        teardownTap()
        healthTimer?.invalidate()
        healthTimer = nil
        waitForPermission()
    ...
    fileprivate func handle(type: CGEventType, event: CGEvent) -> Bool {
        ...
        // The tester sits ahead of the engine on purpose: it has to see buttons no
        // binding claims, buttons an excluded app would skip, and events arriving
        // mid-gesture — otherwise it can only confirm what already works. While it is
        // listening the engine is bypassed entirely, so identifying a button doesn't
        // also fire Mission Control at you. The event is passed through rather than
        // swallowed; the point is to observe the stream, not to alter it.
        if monitor.isActive {
